...
export async function getMovieById(movieId) {
  ...
  try {
    // Check if the movie ID is a TMDB ID
    if (movieId.startsWith('tmdb-')) {
      const tmdbId = parseInt(movieId.replace('tmdb-', ''), 10);
      if (isNaN(tmdbId)) {
        console.error('Invalid TMDB ID format:', movieId);
        return null;
      }
      // Fetch from TMDB
      return getMovieFromTmdbById(tmdbId);
    } else {
      // Assume it's a Supabase/IMDB ID and fetch from Supabase
      ...
      if (error) {
        console.error('Error fetching movie from Supabase:', error);
        return null;
      }
